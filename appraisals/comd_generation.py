...
BATCH = '4324'
WEIGHTAGE = 30    # as of 221223, commander appraisal weightage is at 30%
...
# each 4D should follow CXXXX and should each be a separate string
ooc_pers = {
    'PLT_1': ['C1105'],
    'PLT_2': [],
...
dfs = helpers.excel_to_df('trial.xlsx')    # change input filename here as needed
...
for plt, df in dfs.items():
    new_df = helpers.get_comd_results(df, WEIGHTAGE, ooc_pers[plt])
    mega_df = pd.concat([mega_df, new_df], axis=1)

mega_df.sort_index(inplace=True)
mega_df.to_excel('output.xlsx', header=False)    # change output file name as needed